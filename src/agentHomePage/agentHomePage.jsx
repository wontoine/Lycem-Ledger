...
function AgentHomePage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [policiesRes, claimsRes] = await Promise.all([
        ...
        fetch("http://127.0.0.1:8000/api/agent/claims/", { headers }),
      ]);
      ...
      if (policiesRes.ok) {
        ...
        policiesData = [
          ...
          },
          {
            ...
            Description:
              "Comprehensive coverage for vehicle damage and injury.",
          },
        ];
      }
      ...
      setFetchError("Error connecting to server. Showing cached/mock data.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreatePolicy = async (e) => {
    ...
    // Base Payload
    let payload = {
      CustomerID: newPolicyForm.CustomerID,
      AssignedAgentID: storedUserID,
      planID: selectedPlan.planID,
      PlanName: selectedPlan.PlanName,
      Status: "pending",
    };

    // LOGIC: If Custom Plan (ID 4), use custom inputs. Otherwise use default plan data.
    ...
    console.log("Submitting New Policy:", payload);

    // Simulate Network Request
    setTimeout(() => {
      setProcessing(false);
      alert("Policy Created Successfully (Simulation)");
      ...
      const newMockPolicy = {
        ...
        CoverageLim: payload.CoverageLim,
        BasePrice: payload.BasePrice,
        StartDate: new Date().toISOString().split("T")[0],
        Description: payload.Description,
      };
      setMyPolicies((prev) => [...prev, newMockPolicy]);
              ...
              {claims.length === 0 ? (
                ...
                claims.map((claim) => (
                  ...
                  >
                    ...
                    <td className="p-4 font-mono">{claim.PolicyID}</td>
