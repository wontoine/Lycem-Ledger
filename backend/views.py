...
def _current_user(request):
    """
    Determine the current user from request headers in a robust, case-insensitive way.

    Accepted header names (any casing):
      - x-user-id  (preferred)
      - X-User-ID  (legacy)
      - userid     (legacy)

    We also check Django's WSGI META variants (HTTP_X_USER_ID and HTTP_USERID).

    Returns: User object if found and enabled in DB, otherwise None.
    """
    try:
        # Django's request.headers is case-insensitive, but we also include META fallbacks.
        uid_raw = (
        ...
        # Normalize and validate numeric user id
        ...
        user = User.objects(userid=uid_int).first()
        return user
    except Exception:
        return None
